...
const allowedUpdates = computed(() => Object.fromEntries(
  ALL_UPDATE_TYPES.map((updateType) => [updateType, props.modelValue.includes(updateType)])
));
  ...
  <div class="rounded bg-translucentbackground p-5">
    <div class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
      <div v-for="updateType of ALL_UPDATE_TYPES">
        <input type="checkbox" class="checked:accent-grammy-600" :checked="allowedUpdates[updateType]" @click="set(updateType, $event.target.checked)" />
